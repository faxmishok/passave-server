...
cardSchema.pre('save', function (next) {
  if (this.isModified('bank_website')) {
    let input = this.bank_website?.trim(); // Handle undefined/null cases

    if (!input) {
      // If bank_website is cleared or empty, reset logoURL too
      ...
      try {
        ...
        // Ensure the canonical bank_website has "www." at the beginning
        if (!urlObj.hostname.startsWith('www.')) {
          urlObj.hostname = 'www.' + urlObj.hostname;
        }

        // Store the canonical bank_website (protocol + www.hostname)
